...
<br>
  ...
  <div class="row g-0">
    <div class="col-md-4">
      <img src="../../static/npcs2/varcella17.png" >
    </div>
    <div class="col-md-8">
      <div class="card-body">
        <h5 class="card-title">The begining</h5>
        <p class="card-text">Muita coisa de estilização  do seu personagem</p>
...
$result_usuarios= $pdo-> prepare( "SELECT * FROM shops  WHERE nome_da = 'The_Begin' ORDER BY inv_gold desc LIMIT $qnt_result_pg OFFSET $inicio");
...
echo "<form action='key_12enemy1_bck.php' method='POST'>";
 ...
 echo "</div>";
    ...
    $result_pg= $pdo-> prepare( "SELECT COUNT(inv_id) AS num_result FROM shops WHERE nome_da = 'The_Begin'");
    $result_pg-> execute();
...
foreach($resultado_pg as   $row_pg ) {      
    ...
    $max_links = 2;
     ...
     echo " <a href='key_12thebegin.php?pagina=1'class='page-link' aria-label='Goto page 1'>1</a>";
    for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++){
      if($pag_ant >= 1){

        echo "<li class='page-item'><a class='page-link' href='key_12thebegin.php?pagina=$pag_ant' > Prev:".$pag_ant."</a></li>";
      }
    }
    ...
    echo "<li class='page-item'><a class='page-link' href='key_12thebegin.php?pagina=$quantidade_pg' class='page-link' aria-label='Goto page 86'>Ultima</a></li>";
